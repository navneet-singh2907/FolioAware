...
| T12 | Oversized or repeated requests create a model-cost or availability attack | Denial of service; unbounded consumption | High | Input/body limits; per-process client and global quotas; bounded client state; answer concurrency, provider, request, and instance limits; bounded top-k/tokens; pre-deployment edge controls and billing alerts | `413`, `429`, or `503` |
...
| T24 | CORS is mistaken for authentication or cost protection | Spoofing; denial of service | High | Exact origin allowlist plus ADR-0014 application admission; validate ASGI proxy trust; add deployment-wide edge limits, budgets, alerts, timeouts, and maximum instances before public launch | `429`/`503`; contain spend |
